...
// existing slots (Level1-3) consistently persist into the scene array;
// the new slots may show {fileID: 0}. The script self-verifies after
// saving the scene and, if any slot is null, logs an ERROR listing the
// GUIDs to paste manually into the scene's _levels block. Running the
// script a second time (after assets exist on disk) typically fills the
// array correctly.
//
...
public static class GenerateVerticalSlice
{
    ...
    private static readonly LevelSpec[] Specs = new[]
    {
        ...
        {
            Name = "Level1Config",
            SeqLen = 3, MemTime = 5.0f, Lives = 3,
            Colors = new[]
            {
                PlatformColor.Red, PlatformColor.Blue, PlatformColor.Green
            },
            PerRow = 2, Cactus = 0.00f,
            Spacing = 3.3f, Spread = 7.5f,
            Width = 4.0f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 8, A = 12, B = 18
        },
        new LevelSpec
        {
            Name = "Level2Config",
            SeqLen = 4, MemTime = 5.0f, Lives = 3,
            Colors = new[]
            {
                PlatformColor.Red, PlatformColor.Blue, PlatformColor.Green
            },
            PerRow = 2, Cactus = 0.00f,
            Spacing = 3.4f, Spread = 7.8f,
            Width = 4.0f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 10, A = 14, B = 20
        },
        new LevelSpec
        {
            ...
            Colors = new[]
            {
                ...
                PlatformColor.Green, PlatformColor.Yellow
            },
            PerRow = 2, Cactus = 0.00f,
            Spacing = 3.5f, Spread = 8.0f,
            Width = 3.8f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 11, A = 15, B = 22
        },
        new LevelSpec
        {
            Name = "Level4Config",
            SeqLen = 5, MemTime = 5.0f, Lives = 3,
            Colors = new[]
            {
                PlatformColor.Red, PlatformColor.Blue,
                PlatformColor.Green, PlatformColor.Yellow
            },
            PerRow = 2, Cactus = 0.20f,
            Spacing = 3.6f, Spread = 8.2f,
            Width = 3.8f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 13, A = 18, B = 25
        },
        new LevelSpec
        {
            ...
            PerRow = 2, Cactus = 0.15f,
            Spacing = 3.8f, Spread = 9.5f,
            Width = 3.6f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 14, A = 19, B = 27
        },
        new LevelSpec
        {
            ...
            PerRow = 3, Cactus = 0.25f,
            Spacing = 3.7f, Spread = 9.0f,
            Width = 3.4f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 16, A = 22, B = 30
        },
        new LevelSpec
        {
            ...
            PerRow = 3, Cactus = 0.20f,
            Spacing = 3.8f, Spread = 10.0f,
            Width = 3.4f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.25f,
            S = 18, A = 25, B = 33
        },
        new LevelSpec
        {
            Name = "Level8Config",
            SeqLen = 6, MemTime = 5.0f, Lives = 3,
            Colors = new[]
            {
                PlatformColor.Red, PlatformColor.Blue,
                PlatformColor.Green, PlatformColor.Yellow,
                PlatformColor.Purple, PlatformColor.Orange
            },
            PerRow = 2, Cactus = 0.15f,
            Spacing = 3.8f, Spread = 9.0f,
            Width = 3.6f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.0f,
            S = 20, A = 27, B = 36
        },
        new LevelSpec
        {
            ...
            PerRow = 3, Cactus = 0.40f,
            Spacing = 3.8f, Spread = 9.5f,
            Width = 3.4f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.2f,
            S = 22, A = 30, B = 40
        },
        new LevelSpec
        {
            ...
            PerRow = 3, Cactus = 0.30f,
            Spacing = 3.9f, Spread = 10.0f,
            Width = 3.4f, Height = 0.5f,
            RowStartY = 3f, Jitter = 0.3f,
            S = 25, A = 33, B = 45
        },
    };
    ...
    public static void Execute()
    {
        ...
        // ScriptableObject instance might not survive a serialization
        // round-trip cleanly when written to a scene serialized field).
        for (int i = 0; i < Specs.Length; i++)
        {
            ...
            {
                Debug.LogError(
                    $"[VSlice] Failed to re-load {path} after creation.");
            }
        }
        ...
        var lsc = Object.FindAnyObjectByType<LevelSessionController>(
            FindObjectsInactive.Include);
        if (lsc == null)
        {
            Debug.LogError(
                "[VSlice] LevelSessionController not found in scene.");
            return;
        }
        ...
        // every _levels entry has a non-zero asset reference. If not, log
        // the expected GUID list so it can be pasted manually.
        VerifySceneWiring(levelAssets);

        Debug.Log(
            $"[VSlice] Done. {Specs.Length} TimeRankConfigs + " +
            $"{Specs.Length} LevelConfigs written.");
    }

    private static void VerifySceneWiring(LevelConfig[] expected)
    {
        string sceneText = File.ReadAllText(ScenePath);
        int levelsIdx = sceneText.IndexOf(
            "  _levels:", System.StringComparison.Ordinal);
        if (levelsIdx < 0)
        {
            Debug.LogError(
                "[VSlice] Could not find '_levels:' block in scene file.");
            return;
        }
        ...
        for (int i = 1; i <= expected.Length && i < lines.Length; i++)
        {
            if (lines[i].Contains("{fileID: 0}"))
            {
                ...
                string label = expected[i - 1].name;
                missing.Add(
        ...
        {
            Debug.Log(
                "[VSlice] Scene wiring verified: all "
                + $"{expected.Length} entries persisted.");
            return;
        }

        Debug.LogError(
            $"[VSlice] Scene wiring INCOMPLETE: {nullCount} of "
            + $"{expected.Length} entries are {{fileID: 0}}.");
        Debug.LogError(
            "[VSlice] Run the script again (now that assets exist on "
            + "disk) - that usually fixes it.");
        Debug.LogError(
            "[VSlice] If the second run still leaves nulls, paste the "
            + "following lines manually into the _levels: block in "
            + "Game.unity (replace any {fileID: 0} entries):\n"
            + string.Join("\n", missing));
    }

    // -------------------------------------------------------------------------
    private static TimeRankConfig EnsureRankAsset(
        int levelNumber, float s, float a, float b)
    {
        string path =
            $"{RankFolder}/Level{levelNumber:D2}_TimeRankConfig.asset";
    ...
    private static LevelConfig EnsureLevelAsset(
        LevelSpec spec, TimeRankConfig rank)
    {
        ...
        {
            colorsProp.GetArrayElementAtIndex(i).intValue
                = (int)spec.Colors[i];
        }
