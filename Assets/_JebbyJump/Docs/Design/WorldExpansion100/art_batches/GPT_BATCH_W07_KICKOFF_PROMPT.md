# Paste-Ready Kickoff Prompt — Batch W07 (Clockwork Heights world art)
...
This session executes **batch W07 only**: the complete art family for
**World 07 - Clockwork Heights**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W07_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W07_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W07 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W07 stage: the tower ticks just ahead - near, crisp, mechanisms visible.
...
## Step 3 - produce the 12 W07 assets
All under `Assets/_JebbyJump/Art/Worlds/W07_ClockworkHeights/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_clockworkheights_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_clockworkheights_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_clockworkheights_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_clockworkheights_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_clockwork_spike_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_clockworkheights_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_clockworkheights_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_clockworkheights_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_clockworkheights_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_clockworkheights_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_clockworkheights_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_gearwork_goggles_01.png` | 512x512 | transparent |

World identity: brass towers of turning gears. Mood: inventive, warm-industrial, cozy. Material: brass and painted metal.
Palette: copper + teal (support gold + warm grey). Lighting: warm workshop light from upper-left (golden, not cold steel). Atmosphere: gentle steam puffs and soft glints.
Hazard: **a chunky brass gear-spike / cog with teeth** - rendered in the cute-cartoon language above.
Finale set-piece: a giant gear-face set-piece. Cosmetic reward: **Gearwork goggles**.
...
NO inherent hue** - only material detail (brass and painted metal texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W07_ClockworkHeights/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W07_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W07_zip_manifest.json --dir <out> --out jebby_art_W07.zip`
4. Deliver the single file `jebby_art_W07.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W07` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W07 distance; background meets the
