# Paste-Ready Kickoff Prompt — Batch W06 (Candy Cloud Kingdom world art)
...
This session executes **batch W06 only**: the complete art family for
**World 06 - Candy Cloud Kingdom**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W06_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W06_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W06 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W06 stage: a candy-bright tower on the near skyline - clear, mid-distance, full tiers.
...
## Step 3 - produce the 12 W06 assets
All under `Assets/_JebbyJump/Art/Worlds/W06_CandyCloudKingdom/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_candycloudkingdom_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_candycloudkingdom_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_candycloudkingdom_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_candycloudkingdom_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_candy_thorn_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_candycloudkingdom_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_candycloudkingdom_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_candycloudkingdom_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_candycloudkingdom_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_candycloudkingdom_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_candycloudkingdom_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_frosting_crown_01.png` | 512x512 | transparent |

World identity: a sweet kingdom of frosting clouds. Mood: playful, sugary, joyful. Material: candy and frosting.
Palette: pink + cream + mint (support lilac + lemon). Lighting: soft warm candy glow, bright and even. Atmosphere: sugar sparkle and drifting sprinkles.
Hazard: **a chunky candy-cane thorn / gumdrop spike** - rendered in the cute-cartoon language above.
Finale set-piece: a frosting palace gate. Cosmetic reward: **Frosting crown**.
...
NO inherent hue** - only material detail (candy and frosting texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W06_CandyCloudKingdom/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W06_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W06_zip_manifest.json --dir <out> --out jebby_art_W06.zip`
4. Deliver the single file `jebby_art_W06.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W06` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W06 distance; background meets the
