# Paste-Ready Kickoff Prompt — Batch W10 (Rainbow Tower Castle world art)
...
This session executes **batch W10 only**: the complete art family for
**World 10 - Rainbow Tower Castle**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W10_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W10_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W10 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W10 stage: you have ARRIVED - the Rainbow Tower is the stage itself, filling the scene grandly (still a soft painted backdrop, calm centre band for platforms).
...
## Step 3 - produce the 12 W10 assets
All under `Assets/_JebbyJump/Art/Worlds/W10_RainbowTowerCastle/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_rainbowtowercastle_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_rainbowtowercastle_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_rainbowtowercastle_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_rainbowtowercastle_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_rainbow_ward_spike_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_rainbowtowercastle_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_rainbowtowercastle_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_rainbowtowercastle_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_rainbowtowercastle_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_rainbowtowercastle_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_rainbowtowercastle_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_radiant_heirloom_set_01.png` | 512x512 | transparent |

World identity: the radiant castle at journey's end. Mood: triumphant, radiant, warm, celebratory. Material: rainbow crystal and gold.
Palette: full warm spectrum (support gold + rose + sky). Lighting: radiant golden-hour light, warm and glowing. Atmosphere: gentle rainbow shimmer and soft sparkles.
Hazard: **a chunky rainbow-crystal ward spike** - rendered in the cute-cartoon language above.
Finale set-piece: the tower's grand hall + summit. Cosmetic reward: **Radiant heirloom set**.
...
NO inherent hue** - only material detail (rainbow crystal and gold texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W10_RainbowTowerCastle/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W10_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W10_zip_manifest.json --dir <out> --out jebby_art_W10.zip`
4. Deliver the single file `jebby_art_W10.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W10` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W10 distance; background meets the
