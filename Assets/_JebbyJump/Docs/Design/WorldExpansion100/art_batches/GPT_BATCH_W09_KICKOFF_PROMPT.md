# Paste-Ready Kickoff Prompt — Batch W09 (Stormfire Volcano world art)
...
This session executes **batch W09 only**: the complete art family for
**World 09 - Stormfire Volcano**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W09_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W09_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W09 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W09 stage: the tower looms against the firelit sky - near, crisp, warmly backlit.
...
## Step 3 - produce the 12 W09 assets
All under `Assets/_JebbyJump/Art/Worlds/W09_StormfireVolcano/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_stormfirevolcano_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_stormfirevolcano_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_stormfirevolcano_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_stormfirevolcano_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_volcanic_ember_spike_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_stormfirevolcano_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_stormfirevolcano_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_stormfirevolcano_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_stormfirevolcano_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_stormfirevolcano_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_stormfirevolcano_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_emberguard_mantle_01.png` | 512x512 | transparent |

World identity: ember peaks under a charged sky. Mood: dramatic, warm, heroic, cozy-not-scary. Material: volcanic rock and ember.
Palette: ember orange + deep plum (support gold + warm crimson). Lighting: warm ember glow from below + soft golden sky light (never harsh/scary). Atmosphere: drifting ash motes and warm sparks.
Hazard: **a chunky ember-crystal spike with a warm glow** - rendered in the cute-cartoon language above.
Finale set-piece: an ember summit set-piece. Cosmetic reward: **Emberguard mantle**.
...
NO inherent hue** - only material detail (volcanic rock and ember texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W09_StormfireVolcano/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W09_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W09_zip_manifest.json --dir <out> --out jebby_art_W09.zip`
4. Deliver the single file `jebby_art_W09.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W09` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W09 distance; background meets the
