# Paste-Ready Kickoff Prompt — Batch W05 (Ocean Sky world art)
...
This session executes **batch W05 only**: the complete art family for
**World 05 - Ocean Sky**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W05_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W05_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W05 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W05 stage: the tower stands on a far floating sky-island - clear, mid-distance, full tiers visible.
...
## Step 3 - produce the 12 W05 assets
All under `Assets/_JebbyJump/Art/Worlds/W05_OceanSky/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_oceansky_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_oceansky_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_oceansky_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_oceansky_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_coral_spine_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_oceansky_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_oceansky_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_oceansky_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_oceansky_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_oceansky_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_oceansky_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_tidal_fin_cloak_01.png` | 512x512 | transparent |

World identity: floating reefs where sea meets sky. Mood: dreamy, fresh, buoyant, WARM-lit. Material: coral and sea-glass.
Palette: aqua + warm coral (support seafoam + sunset gold). Lighting: bright warm sunlight from upper-left (sunset-tinted, not cold). Atmosphere: drifting bubbles and soft spray.
Hazard: **a chunky coral spine cluster** - rendered in the cute-cartoon language above.
Finale set-piece: a floating reef spire. Cosmetic reward: **Tidal fin cloak**.
...
NO inherent hue** - only material detail (coral and sea-glass texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W05_OceanSky/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W05_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W05_zip_manifest.json --dir <out> --out jebby_art_W05.zip`
4. Deliver the single file `jebby_art_W05.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W05` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W05 distance; background meets the
