# Paste-Ready Kickoff Prompt — Batch W08 (Moonlit Dreamscape world art)
...
This session executes **batch W08 only**: the complete art family for
**World 08 - Moonlit Dreamscape**. Precise EXECUTION, not redesign.

## Repository access
Public repo `https://github.com/dirkzzz702/JebbyJump` (branch `main`). Fetch via raw URLs, e.g.
`https://raw.githubusercontent.com/dirkzzz702/JebbyJump/main/Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W08_zip_manifest.json`
...
2. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W08_zip_manifest.json` - exact sizes/paths/alpha the validator enforces.
3. `Assets/_JebbyJump/Docs/Design/WorldExpansion100/04_TEN_WORLD_CREATIVE_BIBLE.md` (W08 section) + `17_WORLD_ART_SYSTEM_AND_TECHNICAL_SPECS.md`.
...
W08 stage: the tower shines under the moon, close now - near, crisp, windows lit warm.
...
## Step 3 - produce the 12 W08 assets
All under `Assets/_JebbyJump/Art/Worlds/W08_MoonlitDreamscape/`:

| # | File | Size | Alpha |
|---|---|---|---|
| 1 | `Backgrounds/bg_moonlitdreamscape_01.png` | 2400x1080 | opaque |
| 2 | `Backgrounds/landmark_tower_moonlitdreamscape_01.png` | 900x1400 | transparent |
| 3 | `Floor/floor_moonlitdreamscape_01.png` | 512x128 | opaque |
| 4 | `Platforms/plat_moonlitdreamscape_base_01.png` | 256x96 | transparent |
| 5 | `Obstacles/hazard_moon_thorn_01.png` | 1254x1254 | transparent |
| 6 | `Decorations/deco_moonlitdreamscape_01.png` | 1024x1024 | transparent |
| 7 | `UI/world_thumb_moonlitdreamscape_01.png` | 512x512 | opaque |
| 8 | `UI/world_badge_moonlitdreamscape_01.png` | 192x192 | transparent |
| 9 | `UI/story_card_moonlitdreamscape_01.png` | 1600x900 | opaque |
| 10 | `UI/finale_moonlitdreamscape_01.png` | 1920x1080 | transparent |
| 11 | `UI/world_gem_moonlitdreamscape_01.png` | 256x256 | transparent |
| 12 | `UI/cosmetic_starlace_veil_01.png` | 512x512 | transparent |

World identity: a soft night of stars and dreams. Mood: serene, magical, dreamy, gently WARM. Material: moon-glass and starlight.
Palette: indigo + warm moon-gold (support violet + soft gold). Lighting: warm golden moonlight (honey-toned moon, not cold blue night). Atmosphere: drifting stardust and soft glows.
Hazard: **a chunky moon-crystal thorn cluster** - rendered in the cute-cartoon language above.
Finale set-piece: a star-strung moon bridge. Cosmetic reward: **Starlace veil**.
...
NO inherent hue** - only material detail (moon-glass and starlight texture, soft top-light, rounded edge).
...
1. Write all 12 PNGs into a working dir `<out>`, preserving their full
   repo-relative paths (i.e. `<out>/Assets/_JebbyJump/Art/Worlds/W08_MoonlitDreamscape/...`).
2. Validate:
   `python ProductionArtAudit/tools/validate_generated_art.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W08_zip_manifest.json --dir <out>`
3. On PASS build the zip:
   `python ProductionArtAudit/tools/build_final_art_zip.py --manifest Assets/_JebbyJump/Docs/Design/WorldExpansion100/art_batches/W08_zip_manifest.json --dir <out> --out jebby_art_W08.zip`
4. Deliver the single file `jebby_art_W08.zip` to the folder
   `Downloads/jebby-jump/jebby_art_W08` on the user's machine.
...
tower = the tiered rainbow-disc spire at the W08 distance; background meets the
